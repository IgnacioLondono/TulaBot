...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (queue.node.isPaused()) {
            return interaction.reply({
                embeds: [new EmbedBuilder().setColor('#FFA500').setTitle('⏸️ Ya pausado').setDescription('La música ya está pausada.')],
                flags: 64
            });
        }
        ...
        if (musicQueue.current) {
            await musicSystem.sendMusicEmbed(interaction.guild.id, interaction.channel, musicQueue.current, true);
        }

        return interaction.reply({
            embeds: [new EmbedBuilder().setColor(config.embedColor).setTitle('⏸️ Pausado').setDescription('La reproducción ha sido pausada.')]
        });
    }
};
